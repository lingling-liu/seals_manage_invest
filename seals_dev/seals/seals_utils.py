...
def convert_regional_change_to_coarse(regional_change_vector_path, regional_change_classes_path, coarse_ha_per_cell_path, scenario_label, output_dir, 
                                      output_filename_end, columns_to_process, region_ids_raster_path=None, distribution_algorithm='proportional', coarse_change_raster_path=None,
                                      year = None):
    ...
    # print(f"==========================PROCESSING REGIONAL CHANGE FOR YEAR {year}===========================")
    if year:
        regional_change_classes = regional_change_classes[(regional_change_classes['year'] == year)]
        if len(regional_change_classes) == 0:
            raise ValueError(f"Year {year} not found in regional_change csv file")
        if len(regional_change_classes) > 1:
            raise ValueError(f"Year {year} found multiple times in regional_change csv file")

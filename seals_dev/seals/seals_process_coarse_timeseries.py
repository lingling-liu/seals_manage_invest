            ...
            if p.scenario_type != 'baseline':
                if p.regional_projections_input_path:
                    ...
                    for c, year in enumerate(p.years):   
                        ...
                        reg_year = year if 'A' in pd.read_csv(regional_change_classes_path).columns else None
                        seals_utils.convert_regional_change_to_coarse(regional_change_vector_path, regional_change_classes_path, coarse_ha_per_cell_path, scenario_label, 
                                                                      output_dir, output_filename_end, columns_to_process, region_ids_raster_path=region_ids_raster_path, distribution_algorithm='proportional', 
                                                                      coarse_change_raster_path=None, year=reg_year)
